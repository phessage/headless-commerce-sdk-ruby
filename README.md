...
The client also covers customer auth configuration, password/OTP/native social sign-in, refresh/logout, profile, cart merge, addresses, customer orders/cancellation and returns. Access tokens last 15 minutes and refresh capabilities rotate on every use; keep them in encrypted Rails/session storage. This preview does not directly capture or initiate refunds. Hosted checkout and signed commerce events are platform capabilities. Pass the exact request body and headers to `WebhookVerifier.verify`, keep the `whsec_` secret server-side, and provide a replay store whose `claim` operation is atomic and backed by a unique delivery-ID constraint.
